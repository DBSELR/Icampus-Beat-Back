...
public class SemesterGradeReportService : ISemesterGradeReportService
{
    private readonly IGenericRepository<object> _repo;
    public SemesterGradeReportService(IGenericRepository<object> repo) => _repo = repo;
    ...
    {
        var sql = @"SELECT DISTINCT REGU, '20' + REGU + '-20' + CAST(CAST(REGU AS INT) + (MAXSEM/2) AS VARCHAR) AS BATCH FROM TBL_COURSE WHERE COURSE = @COURSE AND Regulation = @REGULATION";
    ...
    public async Task<IEnumerable<object>> LoadPapersListAsync(SemesterGradePapersRequest request)
    {
        var ps = new[]
        {
    ...
    //public async Task<int> RunRegnoResultProcessAsync(SemesterResultProcessRequest request)
    //{
    //    // preserve original DAL behaviour: drop temp table for specific DB then exec the correct proc (dynamic exec string)
